...
function LessonContent() {
  ...
  return <main className="min-h-screen pb-20 bg-slate-50 dir-rtl"><header className="relative overflow-hidden bg-slate-950 text-white"><div className="absolute inset-0 opacity-30">{(week.image || course.image) && <img src={week.image || course.image} alt="" className="w-full h-full object-cover" />}</div><div className="relative max-w-5xl mx-auto p-5 md:p-9"><button onClick={goBack} className="w-10 h-10 rounded-full bg-white/15 grid place-items-center"><IoArrowBack /></button><div className="mt-7"><p className="font-cairo text-blue-200 text-sm">{course.title}</p><h1 className="font-aref text-4xl font-bold mt-2">{week.title}</h1><p className="font-cairo text-white/80 mt-3 max-w-2xl">{week.description}</p></div></div></header><section className="max-w-5xl mx-auto p-4 md:p-8"><div className="flex items-center justify-between mb-6"><div className="flex gap-2 items-center"><IoPlayCircle className="text-blue-600" size={25} /><h2 className="font-aref text-2xl font-bold">فيديوهات الأسبوع</h2></div><button onClick={() => router.push(`/exams?courseId=${encodeURIComponent(course.id)}`)} className="font-cairo bg-emerald-600 text-white rounded-xl px-4 py-3 text-sm font-bold flex gap-2 items-center"><IoDocumentText />الامتحانات</button></div><VideoSection customVideos={week.videos} /></section></main>;
}
