...
function ExamsContent() {
  ...
  return <main className="min-h-screen dir-rtl bg-slate-50 pb-20"><header className="bg-slate-950 text-white"><div className="max-w-4xl mx-auto p-5 md:p-8"><button onClick={goBack} className="w-10 h-10 rounded-full bg-white/10 grid place-items-center"><IoArrowBack /></button><div className="mt-6 flex items-center gap-3"><span className="w-12 h-12 rounded-2xl bg-blue-500/20 grid place-items-center"><IoDocumentText size={26} className="text-blue-200" /></span><div><p className="font-cairo text-sm text-blue-200">اختبر تقدمك</p><h1 className="font-aref text-3xl font-bold">امتحانات {course.title}</h1></div></div></div></header><section className="max-w-4xl mx-auto p-4 md:p-8">{course.exams?.length ? <ExamPortal courseId={course.id} exams={course.exams} /> : <ExamSection courseId={course.id} courseName={course.title} />}</section></main>;
}
